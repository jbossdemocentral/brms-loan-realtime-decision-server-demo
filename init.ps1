Write-Host "Installing Loan Realtime Decision Server Demo"
...
Write-Host "- creating kie-server user ...`n"
$argList1 = "-a -r ApplicationRealm -u kieserver -p kieserver1! -ro kie-server --silent"
try {
	Invoke-Expression "$JBOSS_HOME\bin\add-user.ps1 $argList1"

	Write-Error "Error occurred during kie-server user account setup."
	exit
}
